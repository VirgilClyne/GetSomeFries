...
if ($.getjson("Neurogram")) {
	...
	$.Dualsub.Settings = $.getjson("Neurogram")?.Dualsub?.Settings ?? $.Dualsub.Settings;
...
if (Settings.type == "Disable") $.done()
    ...
    if (url.match(`lang=${Settings.tl}`) || url.match(/&tlang=/)) $.done()
        ...
        let tl =( Settings.tl == "zh-CN") ? "zh-Hans"
        : (Settings.tl == "zh-TW") ? "zh-Hant"
        : (Settings.tl)
        let url = `${url}&tlang=${tl}`
    } $.done({ url })
} else $.done();
